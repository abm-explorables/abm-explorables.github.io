...
## 確定 copy — HOME hero メッセージ(Yuito gate 通過 2026-06-07)
```
一人一人はごく単純な規則で動いているだけ
それなのに市場全体は誰の予想も超えていく
その瞬間を、ぜひ自分の手で再現してみてほしい
```
- ボトムアップ→創発の意外さ→「自分の手で再現」の参加型ワクワク。命令形スローガンを避け発見の好奇心を煽る方向
- 句読点: 3行とも句点なし、3行目の読点ひとつだけ(リズム確定)
- 「再現」= スライダーで暴落を自分で出すデモ体験と動詞が一致(hero = firefront、cases = Cont-Bouchaud)
...
- **未決の tuning(hero 実装時に live で決める / Yuito, 2026-06-07)**: 「実際の炎(芯が最も明るい)」寄りか「広がる縁を淡く」寄りか。現 mapping は g(火の強度)→ 色で、最前線 g=1=淡青・燃え跡 g<0.5=濃青→background なので**既に外縁が淡い**。最終印象は輝度カーブと BLOOM_GAIN 次第 → 小スケッチでなく hero 実寸で振って判断する

## 追加済み事例(2026-06-07)
- **感染の閾値 — Network SIR**(`/cases/epidemic/`): epidemic.html を EpidemicSIR.astro に蒸留移植。見出しを「感染力と被害は比例する？」に。**臨界バグを調査して修正**(N/R/γ 再調整 + R0 をパーコレーション較正 T/T_c へ)。経緯は notes/epidemic-criticality-fix.md
- **幻の渋滞 — Phantom Jam**(`/cases/traffic/`): traffic.html を TrafficJam.astro に蒸留移植(OVM、sim 無改変)
- 両者とも ContBouchaud と同一の蒸留(eyebrow/pill/gradient card/italic/mono UI 除去)。HOME 事例リストに論文引用付きで追加

## Parked 候補(将来の事例で再利用 — Yuito 2026-06-07)
hero には不採用だが将来の事例ページで使う意図。public/sketches/ に tracked 保持(URL で閲覧可)。
- **contagion**(連鎖破綻 / ネットワークカスケード)→ systemic-risk / 連鎖破綻の事例 hero 候補。現状は赤(crash)版
- **murmuration**(群飛 / Vicsek 相転移)→ herding / flocking の事例、または区切りの transition。※重さ対処(周期リセット)は採用時に
- sandpile / physarum は不採用・未保持(git 履歴 c9e79d6 にのみ存在、必要時に復元)
