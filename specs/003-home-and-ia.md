...
## signature moment 候補(プロトタイプ比較中)
public/sketches/ に WebGL スケッチを並べて Yuito が目視選定。全候補 navy field + データ意味論色のみ(locked system 準拠)。
- murmuration(群飛 / Vicsek 相転移)— Yuito 好感触
- contagion(連鎖破綻 / ネットワークカスケード)— 本命候補(systemic risk 直球)
- firefront(延焼 / 森林火災 SOC)
- sandpile(砂山 SOC)— 怖い、脱落
- physarum(粘菌)— キモい、脱落

**採択された1つだけ**を本実装で磨く(不採択への変更はトークンの無駄 = Yuito 指示)。採択時の既知 TODO:
- murmuration を採れば: **時間経過で重くなる**(coherence 上昇→群れ密集→1セル近傍数が膨張→近傍ループが重い)。対処は「一定周期/一定コヒーレンスで状態をソフトリセットして最初に戻す」+ 任意で per-cell 近傍数キャップ。**採択時のみ着手**
