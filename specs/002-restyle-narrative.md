...
Status: 完了(2026-06-07。style + ヒステリシス deploy 済み・Yuito 確認「いい感じ」、copy は gate 通過「基本〇」で反映済み。badge(sub) は「安定」を採用)
...
- **navy 原則**(Yuito, 2026-06-07 追記 — notes/003-reference-extraction.md): navy は neutral anchor(最暗ベース + off-white + くすんだ navy グレー)で field を静かに保つ。**色相が pop してよいのはデータ意味論を背負う箇所のみ**。禁じ手 = navy gradient / navy 塗り装飾 / navy バッジ。これに伴い h1 em の calm 色(意味を背負わない hue pop)も除去
...
## Appendix: script 変更行の列挙(Sim 不変例外の監査ログ)
1. `regime()` — 戻り値オブジェクトを `REGIMES` 定数に括り出し(挙動同一、ヒステリシスから cls 参照するため)
2. `paintUI()` — regime 表示ヒステリシス追加(`REGIME_HOLD_MS = 1500`)。旧「即時切替」ブロックを置換。badge / story / regime 色連動は安定化後の regime に従い、数値そのものは毎フレーム更新を維持
3. `drawDist()` 1行 — ヒストグラム本体色 `rgba(122,150,210,.55)` → `rgba(90,134,214,.55)`(= --calm。色値のみ)
4. `step()` / `stats()` — **変更なし**
5. copy 反映(gate 通過後): `REGIMES.sup.txt` 群衆支配→超臨界、`stories.sup` テールリスク文の修正 — いずれも文言のみ
