...
## 5. 補正 (Yuito feedback, 2026-06-07) — destination の選び方

**「AIっぽくない」≠「academic っぽい」。** distill は ML 研究者向けに最適化された論文グラマーで、一次オーディエンス(金融サラリーマン、研究者ではない)には冷たく・敷居高く映りうる。SaaS-LP slop を捨てた先が academic dryness では、AI 臭は消えても肝心の読者を落とす。

参照サイトのトーン軸での再整理:
- **ncase (Polygons)** = 温かい・物語的・広い読者向け → **トーンの手本はこっち**
- **distill** = 厳密・研究者向け → breakout グリッド等の**「機構」だけ盗む。トーンは持ち込まない**
- **Complexity Explorables (Brockmann)** = ABM そのもので題材最近接 → 視覚ではなく**モデルの見せ方・interaction の運び**を盗む対象

**メタ警告: restyle は AI に美的方向性を投げるのが一番危険なフェーズ。**「distill 風にして」と投げると slop に逆戻りするか、distill のパスティーシュ(= 新種の slop)になる。非・模倣可能なコンテンツ判断(データ意味論の配色、文中インライン widget の配置)が差別化であり、それは Yuito が決める領域。**AI は実装に使い、方向性の決定には使わない。**
